...
namespace sfr {
    namespace fmc {                                 // Flash Memory Control Registers
        ...
        union ispcmd_t {                            // ISP Command Register (ISPCMD)
            ...
            sfb_t<ispcmd_t, 0, 1> FCTRL0;
            sfb_t<ispcmd_t, 1, 1> FCTRL1;
            sfb_t<ispcmd_t, 2, 1> FCTRL2;
            sfb_t<ispcmd_t, 3, 1> FCTRL3;
        ...
        union fatcon_t {                            // Flash Access Time Control Register (FATCON)
            __SFR(fatcon_t, uint32_t, 0)
            sfb_t<fatcon_t, 4, 1> LFOM;
        };

        struct fmc_t {
            volatile sfr_t<ispcon_t> ISPCON;        // ISP Control Register (ISPCON)
            const void* volatile ISPADR;            // Data Flash Base Address Register (DFBADR)
            ...
            volatile sfr_t<fatcon_t> FATCON;        // Flash Access Time Control Register (FATCON)
        };
