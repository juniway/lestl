...
namespace sfr {
    namespace i2c {
        ...
        union con_t {                               // I2C Control Register (I2CON)
            ...
            sfb_t<con_t, uint32_t, 2, 1> AA;
            sfb_t<con_t, uint32_t, 3, 1> SI;
            sfb_t<con_t, uint32_t, 4, 1> STO;
            sfb_t<con_t, uint32_t, 5, 1> STA;
            sfb_t<con_t, uint32_t, 6, 1> ENSI;
            sfb_t<con_t, uint32_t, 7, 1> EI;
        };

        union addr_t {                              // I2C Slave Address Register (I2CADDR)
            __SFR(addr_t, uint32_t, 0)
            sfb_t<addr_t, uint32_t, 0, 1> GC;
            sfb_t<addr_t, uint32_t, 1, 7> ADDR;
        };

        union toc_t {                               // I2C Time-Out Counter Register (I2CTOC)
            __SFR(toc_t, uint32_t, 0)
            sfb_t<toc_t, uint32_t, 0, 1> TIF;
            sfb_t<toc_t, uint32_t, 1, 1> DIV4;
            sfb_t<toc_t, uint32_t, 2, 1> ENTI;
        };

        union adrm_t {                              // I2C Slave Address Mask Register (I2CADM)
            __SFR(adrm_t, uint32_t, 0)
            sfb_t<adrm_t, uint32_t, 1, 7> ADM;
        };
        ...
        extern i2c_t I2C;   SFR_ADDR(I2C, I2C_ADDR);
    }
}
